...
\item Scott Cunningham, Ben H. Williams Professor of Economics at Baylor University in Texas USA (on loan for a year at Harvard)
...
\item High energy, eclectic approach to teaching based largely on what I need in order to learn things
\item Technical econometrics, history of thought, videos, applications, code, spreadsheets, exercises
...
Basics of difference-in-differences
	...
	\item DiD equation as 2x2 (``four averages and three differences'', 2x2), parallel trends and estimation with OLS specification and inference
...
Pathologies of and Solutions to Twoway Fixed Effects with Differential Timing
	\begin{itemize}
	\item Panel fixed effects and strict exogeneity
...
\item Applications with data
	\begin{itemize}
	\item Brazilian mental health reform
	\item Covariates and staggered adoption
	\end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Selection based on beliefs about post-treatment information}
    \begin{itemize}
...
\item ``No anticipation'' simply means that the unit is not treated until it is treated (and that can be violated with rational forward looking agents but not always)
	...
	\end{itemize}
\item Violations in simple 2x2 where baseline is treated and this creates problems in part because parallel trends embeds untreated baselines
...
\begin{frame}{Four Ways to Estimate a 2x2 DiD}
There are actually \textbf{four} ways to estimate this simple 2x2 design:

\begin{enumerate}
    \item \textbf{Manually}, using the four sample means and the three subtractions.
    \item \textbf{OLS regression with a constant}:
        $Y_{it} = \beta_0 + \beta_1 \text{treat}_i + \beta_2 \text{post}_t + 
        \beta_3 (\text{treat}_i \times \text{post}_t) + \varepsilon_{it}$
    \item \textbf{OLS with fixed effects}, absorbing the treatment main effect.
    \item \textbf{First differences}: 
        $\Delta Y_i = \gamma_0 + \gamma_1 \text{treat}_i + u_i$
\end{enumerate}

\vspace{0.25cm}

The advantage of OLS is that we can lean on its well-known 
\textbf{statistical properties} — inference, standard errors, clustering, and extensions all come along for free.
\end{frame}

\begin{frame}{The Three Regression Versions}
Here are the three regression specifications for the same 2x2 DiD:

\small
\begin{enumerate}
    \item \textbf{Canonical OLS with a constant} \\
    $Y_{it} = \beta_0 + \beta_1 \text{treat}_i + 
    \beta_2 \text{post}_t + 
    \beta_3 (\text{treat}_i \times \text{post}_t) + \varepsilon_{it}$

    \item \textbf{OLS with unit and time fixed effects} \\
    $Y_{it} = \alpha_i + \delta_t + 
    \beta_{\text{DID}} (\text{treat}_i \times \text{post}_t) + \varepsilon_{it}$
...
All three give the same ATT in this simple 2x2 setting.
\end{frame}

\begin{frame}{A Visual Summary}
\centering
\includegraphics[width=\textwidth]{lecture_includes/jel_figure}

\vspace{0.3cm}
We will review all four estimators together in both Stata and R.
\end{frame}
