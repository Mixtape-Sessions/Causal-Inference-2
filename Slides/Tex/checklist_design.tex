...
\item No right or wrong answer to this -- the control group could be either and requires thinking about \emph{selection}
\end{itemize}
...
\begin{frame}{Switching equation vs selection}

\begin{itemize}
\item Switching equations \emph{select} between potential outcomes 
...
\begin{itemize}
\item Not all forms of selection are a problem for parallel trends, but some will destroy them, and this is not always detectable -- they must be discerned other ways
    ...
    \item Ghanem, Sant'Anna, and Wüthrich (2023) show that parallel trends presupposes only seven treatment assignment mechanisms
\end{itemize}
\end{frame}
...
\begin{frame}{Parallel Trends is Consistent with Certain Assignment Mechanisms}
\begin{enumerate}
    \item[2. ] \textbf{Selection on pre-treatment covariates}
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item[3. ] \textbf{Selection on fixed effects}
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item[4. ] \textbf{Selection on Baseline $Y^0$}
    \begin{itemize}
        \item Treatment is assigned based on falling above or below some threshold in baseline $Y^0$ does not violate parallel trends
    \end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{./lecture_includes/selection_y0b_es}
\end{figure}
...
\begin{enumerate}
    ...
    \item[46. ] \textbf{Selection on imperfect beliefs about returns}
    \begin{itemize}
