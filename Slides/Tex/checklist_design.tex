...
\item My name is Scott Cunningham
\item I work at Baylor University in the economics department where I am the Ben H. Williams Professor
\item I live in Waco, Texas, halfway between Dallas and Austin (the state capital)
\item And I am grateful to be here in such a beautiful country, on this amazing continent, in this beautiful city, and this interesting university
\end{itemize}

\end{frame}

\begin{frame}{What should you expect}

\begin{figure}
    \centering
    \includegraphics[height=0.9\textheight]{./lecture_includes/schedule}
\end{figure}
...
% Slide 6
\begin{frame}{Introducing Chained Diff-in-Diff}
  \begin{itemize}
    \item \textbf{Bellego (2024):} Chained DiD for imbalanced panels.
    \item Uses overlapping observations to chain short-term DiDs.
    \item Recovers long-term treatment effects despite dropout.
  \end{itemize}
\end{frame}

% Slide 7
\begin{frame}{Illustration: NLSY97 Panel}
  \begin{itemize}
    \item Youth panel tracked over time with attrition.
    \item College attendance as treatment.
    \item Dropout may be correlated with unobservables (e.g., incarceration risk).
    \item Standard DiD may fail.
  \end{itemize}
\end{frame}

% Slide 8
\begin{frame}{Chaining the Estimates}
  \begin{itemize}
    \item Leverages overlap: e.g., 2000--01, 2001--02, 2002--03.
    \item Estimate short-term DiDs and link them.
    \item Avoids discarding data to force balance.
  \end{itemize}
\end{frame}
...
\begin{frame}{Assumptions and Limitations}
  \begin{itemize}
    \item Assumes conditional parallel trends.
    \item Requires unobservable types to trend similarly within treatment status.
    \item Assumes observables capture drivers of missingness.
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    \item Imbalance isn't inherently a threat—but must be examined.
    \item MAR and conditional MAR offer partial safeguards.
    \item Chained DiD is a flexible, efficient alternative for panels with attrition.
    \item Beware assumptions on unobservables and sampling.
  \end{itemize}
\end{frame}
...
\item If these covariates are themselves predictive of trends, then it is suggestive parallel trends could be \emph{mechanically} breaking, so check $$X_{it} = \alpha + \beta_1 D_i + \beta_t Post_t + \delta ( D_i + Post_t ) + \varepsilon_{it}$$ 
...
\begin{itemize}
\item Potential outcomes define treatment effects, but they are unknown and fixed 
\item Once the treatment assignment happens, we only observe one of the two potential outcomes
 $$Y_{it}=D_{it}Y_{it}^1 + (1-D_{it})Y_{it}^0$$
\item How those treatment get assigned is called the \textbf{treatment assignment mechanism}, or "selection"
...
\begin{itemize}
    \item Treatment assignment versus treatment assignment mechanism mechanisms are distinct concepts
    \item When designing a study, it's essential to try to understand not just \emph{who} was treated, but \emph{why} they, and not everyone, were treatd
    \item One must be willing, eager and successful in building a realistic, historical case
    \item Reason is that Ghanem, Sant'Anna, and Wüthrich (2023) show that parallel trends presupposes only five treatment assignment mechanisms
\end{itemize}
\end{frame}

\begin{frame}{Parallel Trends is Consistent with Certain Assignment Mechanisms}
\begin{enumerate}
    \item \textbf{Constant unit-level trends in $Y^0$}
	\begin{itemize}
	\item Recall that the ATT is missing $Y^0$ for the treatment group
	\item If every unit in the dataset, even the missing ones, would have changed by the same amount period to period
	\end{itemize}
...
\begin{enumerate}
    ...
    \item[4. ] \textbf{Selection on imperfect beliefs about returns}
    \begin{itemize}
    ...
    \item[5. ] \textbf{Martingale property}
    \begin{itemize}
...
\item Parallel trends \emph{does} require that the comparison group follows a trend in outcomes that is approximately the same as the counterfactual $Y^0$ trend of the treatment group 
\end{itemize}
...
\item Parallel trends is \emph{untestable} and to believe me, we will look at an example (inspired by a study of mine later)
...
\item Some forms of selection that satisfy parallel trends will complicate event study plots
\item Consider what you hope to find -- something like selection on fixed effects (first event study)
	\begin{itemize}
	\item Zip codes in the upper half get treated
	\item Zip codes in the lower half don't
	\end{itemize}
\item Then compare that with selection on changes in baseline $Y^0$ (second event study)
	\begin{itemize}
	\item Only people who enroll in the program were the ones who saw sudden drops in earnings
	\end{itemize}
\end{itemize}

\end{frame}

\begin{frame}{Selection on fixed effects and event studies}

\begin{figure}
    \centering
    \includegraphics[height=0.30\textheight]{./lecture_includes/selection_fe.png}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[height=0.30\textheight]{./lecture_includes/selection_y0.png}
\end{figure}
...
\begin{frame}{Event studies are evidence, not proof}

\begin{itemize}
\item Event studies are mandatory -- you must make them
\item But remember, they are not testable, they are not proof; rather, they are evidence
	\begin{itemize}
...
\begin{figure}
    \centering
    \includegraphics[height=0.85\textheight]{./lecture_includes/python_births.png}
\end{figure}
...
\begin{itemize}
\item Covariates in diff-in-diff aren't "omitted variable bias"; rather they repair your broken parallel trends
\item Two parallel trends assumptions
	\begin{enumerate}
\item Unconditional parallel trends:  parallel trends holds for treatment and control (think randomized treatment)
\item Conditional parallel trends: means only units that are similar on covariates would have evolved similarly (think selection on covariates)
	\end{enumerate}
...
\begin{itemize}
\item Consider what an expert on your outcome at that time period as your data would have said was the "ordinary determinants of the outcome" (e.g., birth rates, murders)
...
\item Ultimately then there will be two steps
	\begin{enumerate}
	\item Which covariates \emph{cause} differential trends in $Y^0$ at this time period?
	\item And do they have different means in your treatment and control group?
	\end{enumerate}
\item Let's take them in turn
\end{itemize}
...
\begin{itemize}
\item Since you are missing $Y^0$, ask yourself, "what are the ordinary determinants of US murder from 1977 to 1992?"
	\begin{itemize}
	\item Drugs and gangs (crack cocaine)
...
\begin{frame}{Checking Covariate Imbalance}

\begin{itemize}
\item How do you know if you need to control for the covariates that cause $\Delta Y^0$?
\item Think about the RCT 
	\begin{itemize}
	\item Lots of things cause the outcome, not just the treatment
	\item You don't \emph{need} to control for them though, but why?
	\item Because in an RCT, covariates are on average the same in both groups
	\end{itemize}
...
\item Think of there being two options with differential timing 
	...
	\end{enumerate}
\item Which one do you want? Let's review now the differential timing
\end{itemize}
...
\begin{itemize}
\item First goal is to understand them, their assumptions, their mechanics
\item Second goal is to present a simple criteria for selecting between them
\end{itemize}
