...
\item But we worked with "unconditional parallel trends" which means this:
	...
	$$ \bigg ( \textcolor{red}{E[Y^0_k|Post]} - E[Y^0_k|Pre] \bigg ) - \bigg ( E[Y^0|U|Post] - E[Y^0_U | Pre] \bigg )$$
...
\subsection{Check for Imbalance}
...
\begin{frame}
    ...
    \begin{itemize}
        \item You want to compare apples to apples on observables that are highly predictive of $Y^0$.
        \item This is theoretically driven but also common sense: ask yourself if the two groups would have similar trends.
        \item For example, rural vs. urban counties may have different trends due to different demographics and economics.
    \end{itemize}
\end{frame}
