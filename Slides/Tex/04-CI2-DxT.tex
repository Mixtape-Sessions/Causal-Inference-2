...
\begin{figure}
\includegraphics{./lecture_includes/paul_did_new.png}
\end{figure}

\end{frame}

\begin{frame}{Learn the New Diff-in-Diff}

\begin{itemize} 
\item It's in your best interest to make the fixed cost investment in the new econometrics of DiD as they are designed for modal situations
\item You're probably going to write a paper using DiD at least once in your life
\item Even if you don't, you're going to read a lot of papers using DiD, referee them, or advise students using them
\item Lots of surveys have been written (Roth, et al. 2023; Baker, et al. 2025) and you'll have the slides and recordings here
...
\begin{frame}Design, Comparisons and Covariates}

\begin{itemize}
\item Still important, though, to carefully think about \emph{who} your comparison group is
\item Recall the sign flip with our Craigslist paper with the inclusion of only \emph{one covariate}
\item Don't control for covariates that could be affected by the outcome 
\end{itemize}

\end{frame}

\begin{frame}{Design and Comparisons}

\begin{itemize}
\item Heterogenous treatment effects is the main driver of this renaissance in diff-in-diff
\item Most studies do not allow for already treated groups to be controls, but TWFE does this automatically and unknowingly
...
\item But remember -- the point is not to use diff-in-diff, but rather to provide credible answers to good questions
\item If you do not believe in parallel trends, consider moving in a different direction like synthetic control -- my third workshop coming up!
\end{itemize}
