...
Similar to dcDh which also focused on individual treatment effect as the building blocks.  But the ATT will be weighted aggregates.
...
\textbf{Assumption 1: Parallel trends}. Expressed as an additive model of unit and time fixed effects 

$$Y_{it}(0) = \alpha_i + \beta_t + \varepsilon_{it}$$with $$E[\varepsilon_{it}]=0$$

Only imposes restrictions on $Y(0)$, not treatment effects themselves. Notice how their parallel trends uses \emph{unit} level parallel trends (like in the simulation) which is stronger than the $E[\Delta Y^0|D=1] = E[\Delta Y^0|D=0]$ parallel trends of CS and SA.
...
When A3 is not tenable or not really ex ante justified by theory then we should be worried. There's an A3' that is a slight modification. 
...
\item It's caused by ``forbidden comparisons'' (late to early treated) -- we saw this with Goodman-Bacon (2022)
...
\begin{frame}{Regression vs synth weights}
	\begin{figure}
	\includegraphics[scale=0.35]{./lecture_includes/abadie2015_table1.png}
	\end{figure}
\end{frame}
...
Only units that are alike in observables and unobservables should produce similar trajectories of the outcome variable over extended periods of time. Abadie warns against using it if the fit is poor. 
