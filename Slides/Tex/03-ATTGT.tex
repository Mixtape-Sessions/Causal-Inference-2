...
\begin{frame}{When not to use it}
	...
	\begin{itemize}
		\item Reverse causality: Becker predicted police reduce crime, but when you regress crime onto police, it's usually positive
			\begin{itemize}
			\item TWFE requires an assumption that rules out reverse causality
			\item For those, you typically need an instrument
			\end{itemize}

		\item Time-varying unobserved variables that are correlated with the treatment and the outcome
			\begin{itemize}
			\item It's the time-varying unobservables you have to worry about in fixed effects
			\item Can include time-varying controls, but as always, don't condition on an outcome (i.e., collider)
			\end{itemize}
	\end{itemize}
\end{frame}
...
\subsection{Warning About Interpreting Event Study Coefficients}
...
\item In CS and dCDH, there are two ways you can calculate the pre-treatment coefficients once you have obtained the ATT(g,t) parameter estimates
	\begin{enumerate}
	\item \textbf{Short gap}.  Uses a "rolling" method in which a new baseline is used for each 2x2 $$E[Y|D=1,t-3] - E[Y|D=1,t-2] - E[Y|D=0, t-3, t-2]$$
	\item \textbf{Long difference}.  Uses a "universal baseline" in which the baseline is fixed at $t-1$. $$E[Y|D=1,t-3] - E[Y|D=1,t-1] - E[Y|D=0, t-3, t-1]$$
	\end{enumerate}
\item TWFE and imputation methods that we will discuss next always use a fixed universal baseline, so if you were to use CS or dCDH and want it to be comparable, be sure to use the universal base syntax in Stata, R or python
\end{itemize}
